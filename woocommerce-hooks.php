...
function sngmbh_woo_product_panels(){
	...
	woocommerce_wp_checkbox( array(
		...
		'label'       => 'Is a ticket sales - DEPRECATED',
	...
	*/
	...
	woocommerce_wp_text_input([
		...
		'label'       		=> 'End date event - DEPRECATED',
		'type'				=> 'date',
		'custom_attributes'	=> ['data-type'=>'date'],
	...
	woocommerce_wp_textarea_input( array(
		...
		'label'       => 'Print this on the ticket - DEPRECATED',
...
function sngmbh_woocommerce_order_status_changed($order_id,$old_status,$new_status) {
	...
	if ($new_status == "cancelled" || $new_status == "wc-cancelled" || $new_status == "wc-refunded" || $new_status == "refunded") {
		...
		$freeCodes = $sngmbhSerialcodesValidator->getOptions()->isOptionCheckboxActive('wcRestrictFreeCodeByOrderRefund');
		if ($freeCodes) {
			$order = wc_get_order( $order_id );
			foreach ( $order->get_items() as $item_id => $item ) {
...
final class sngmbhSerialcodesValidator_WC {
	...
	private function addJSFileAndHandler() {
		...
		wp_localize_script(
 			'SngmbhSerialcodesValidator_WC_frontend',
			'SngmbhSerialcodesValidator_phpObject', // name der js variable
 			[
 				'ajaxurl' => admin_url( 'admin-ajax.php' ),
